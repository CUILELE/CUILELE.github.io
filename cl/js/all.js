$(function (){
	...
	if($('.menu_item')[0]){
		alert(1)
		mttenL($('.box1')[0],500,'top',0,'easeOut','')
	}
	...
	if($('.anliecontent_box')[0]){
		mttenL($('.anliecontent_left')[0],700,'left',0,'backOut');
		mttenL($('.anliecontent_center')[0],700,'right',343,'backOut',function (){
			mttenL($('.anliecontent_right')[0],800,'right',50,'backOut');
			mttenL($('.more_anlie')[0],800,'bottom',-100,'backOut');
		});
	}
	//服务
	if($('.serve_content')[0]){
...
//			$(elem).find('img').eq(0).style.zIndex = 22;
		...
		setInterval(function(){
			mttenL($('.serve_content_ul').children()[num],500,'top',0,'backOut')
			num++;
			if(num>=4){
				clearInterval(timer)
				num =0;
